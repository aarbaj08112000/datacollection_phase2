var COUNTER_SHOW_FLOAT = false;  // true = float, false = integer
var COUNTER_DECIMALS   = 2;      // decimals only used when float is enabled
  ...
  $.fn.countTo = function (options, element) {
    ...
    return $(this).each(function () {
      ...
      function render(value) {
        ...
        // Integer mode
        if (!COUNTER_SHOW_FLOAT) {
          value = Math.round(value);
          var formattedValue = counter.numberFormate(value);
          $self.html(formattedValue);
          return;
        }

        // Float mode
        var formattedValue = counter.numberFormate(value.toFixed(COUNTER_DECIMALS));
        $self.html(formattedValue);
      }
